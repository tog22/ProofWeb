...
function print_ajax_buttons ($p, $b, $c) { // could add TEST as default for $a
	...
	print '<span class="buttons">';
	if ($b == ACCEPT) {
		print '<span class="tick-on button"><a title="Take this back" class="'.$p.'" href="set-attitude.php?p='.$p.'&amp;a=NULL"></a></span> ';
	} else if ($c == ACCEPT) {
		print '<span class="tick-c button"><a title="Accept" class="'.$p.' committed" href="set-attitude.php?p='.$p.'&amp;a=1"></a></span> ';
	} else {
		print '<span class="tick-off button"><a title="Accept" class="'.$p.'" href="set-attitude.php?p='.$p.'&amp;a=1"></a></span> ';
	}
	if ($b == REJECT) {
		print '<span class="cross-on button"><a title="Take this back" class="'.$p.'" href="set-attitude.php?p='.$p.'&amp;a=NULL"></a></span> ';
	} else {
		print '<span class="cross-off button"><a title="Reject" class="'.$p.'" href="set-attitude.php?p='.$p.'&amp;a=2"></a></span> ';
	}
	print '</span>';
}
...
function create_implication ($c, $p1, $p2 = 'NULL', $p3 = 'NULL', $p4 = 'NULL') {
	...
	if (!mysql_query("INSERT INTO implications (p1,p2,p3,p4,c) VALUES ($p1,$p2,$p3,$p4,$c)")) { ?>
		<db-error>
			<query>
				<? print "Failed: INSERT INTO implications (p1,p2,p3,p4,c) VALUES ($p1,$p2,$p3,$p4,$c)"; ?>
			</query>
			<error>
				<? print mysql_error(); ?>
			</error>
